...
管理 API：应用 CRUD、菜单草稿编辑、发布/回滚、授予/撤销租户应用、平台授权列表。这些全局变更统一在 `__platform__` 命名空间授权；租户成员读取自身已授权应用使用独立的租户作用域列表接口，不能复用平台管理接口。更新和撤销均使用乐观锁；发布菜单版本使用 `application_id` 粒度的共享 Redsync 锁，避免并发发布，锁内数据库事务尽量短。

页面菜单的 `component` 是控制台页面键而不是可执行模块地址，必须使用 `<application.code>.<page>` 命名空间。application-service 在写入边界校验路由、非空组件和应用前缀，控制台菜单编辑器只列出当前应用 manifest 中的页面；尚未随当前控制台发布、但命名空间合法的既有页面会被保留并标为未安装。外部页面使用独立 `external` 类型及受限 HTTP(S) URL，不能借组件字段绕过白名单。
