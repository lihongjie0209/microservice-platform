...
| --- | --- | --- | --- | --- |
| identity | 登录、TOTP MFA 挑战/恢复码、管理员一次性密码恢复、刷新、登出、用户和服务账号管理、JWKS | 用户/会话/租户 Token/服务账号查询 | 用户、会话、密码/MFA 状态、服务账号事件 + outbox | 摘要化单次挑战、TOTP 防重放、密钥加密、密码恢复令牌重签失效、会话撤销、登录防爆破、乐观锁、审计主体 |
