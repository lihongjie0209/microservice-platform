...
审计日期：2026-09-03。本文记录可重复验证的已交付能力；后续能力按真实业务增长触发，不预建无边界空壳。

## 服务能力

| 服务 | 前端 POST+JSON API | 内部 gRPC | 领域事件 / 消费 | 核心持久化与并发约束 |
| --- | --- | --- | --- | --- |
| identity | 登录、TOTP MFA 挑战/恢复码、管理员一次性密码恢复、刷新、登出、用户和服务账号详情与管理、JWKS；管理变更前刷新详情 | 用户/会话/租户 Token/服务账号查询 | 用户、会话、密码/MFA 状态、服务账号事件 + outbox | 状态切换保留操作者确认的目标状态并拒绝反向切换；摘要化单次挑战、TOTP 防重放、密钥加密、密码恢复令牌重签失效、会话撤销、登录防爆破、乐观锁、审计主体 |
| tenant | 租户、成员、组织树、邀请、用户组、配额；组织树按子节点懒加载、搜索命中项及祖先、批量名称回填 | 完整管理与成员/组织范围查询 | 租户和成员事件；消费组关系用于授权投影 | Schema 隔离、配额原子消费、乐观锁、审计字段；组织查询在数据库层按租户、深度、节点数和批次有界执行，不向浏览器物化全树 |
| authorization | 权限、角色、绑定、RBAC/ABAC、批量决策 | 管理、决策、数据范围、缓存失效 | 权限变更事件；消费租户组事件 | CEL 条件、缓存版本、事务 outbox、乐观锁 |
| audit | 当前应用记录查询和导出 | 记录、查询、导出 | durable 消费 `platform.>`，事件 ID 幂等 | 应用授权、按持久化归属校验、月分区、默认分区、保留/归档、脱敏 |
| config | 草稿、详情、审批/拒绝、发布、回滚、解析；变更操作先刷新详情版本 | 同等生命周期、详情与查询契约 | 配置变更事件 + outbox | 持久化租户和应用授权校验、环境/租户/服务作用域、审批分离、乐观锁 |
| notification | 模板、供应商路由、发送、供应商回执、状态查询；模板和路由编辑前读取最新详情 | 同等供应商/模板详情、发送和回执契约 | 投递状态事件 + outbox | 详情按持久化租户/应用归属二次校验；供应商只引用预配置可靠 upstream、按应用和渠道优先级路由；幂等、重试/死信、频控、回执去重、乐观锁和审计字段 |
...
| data-export | 应用级 Provider 数据集目录与描述、按 Provider schema 生成筛选条件、异步任务创建、查询、分页、取消、重试、下载 | 中央 Export API；携带租户/应用、cursor 和 snapshot token 的可恢复流式 Export Provider 协议 | 携带应用作用域的请求及成功/失败/取消/重试/过期事件 + Outbox | 应用授权、创建时服务端能力/格式/字段/查询白名单与类型校验、作用域幂等、数据库原子 claim、批次提交点续传、异常 EOF 截断保护、分层对象键、流式 S3/MinIO、到期清理、乐观锁 |
...
```text
make verify                    # 本机：race、vet、Proto、Swagger、Helm、Compose 配置校验
make test-integration          # 本机：只编译 integration build tag，不启动容器
GitHub platform-ci             # CI：各服务隔离 Testcontainers + 平台 system-tests
```
