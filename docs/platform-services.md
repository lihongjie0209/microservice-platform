...
- 登录防爆破、会话撤销和设备会话
- 可选 TOTP MFA：首次启用必须校验当前密码和首个动态码；密钥使用用户绑定的 AES-GCM 密文保存，恢复码仅保存独立 pepper 的 HMAC 摘要
- 启用 MFA 的密码登录只签发短时、单次、数据库支持的挑战，不提前创建会话或返回 Token；挑战消费、TOTP 防重放或恢复码消费与会话创建在同一事务完成
- MFA 挑战原文不落库，过期/已消费记录由 identity worker 有界清理；启用或停用 MFA 会撤销当前会话之外的所有会话
- 恢复码可在再次校验当前密码、未重放 TOTP 和期望版本后整批轮换；旧码失效、TOTP 步进、乐观锁更新和 Outbox 事件在同一事务提交，新码仅返回一次
- 用户遗失认证器和恢复码时，平台管理员可在独立 `identity.user.mfa-reset` 权限下查询 MFA 状态并提交带原因和期望版本的恢复操作；重置、恢复码清除、全部会话撤销与 Outbox 事件原子提交，事件携带明确的管理员重置类型和审计原因，即使没有活跃会话也不会丢失恢复证据；管理员不能用该接口绕过自己的自助验证
- 已登录用户可校验当前密码后原子更新 Argon2id 凭证；保留当前会话、撤销其他有效会话，并在同一事务写入密码变更 Outbox 事件
- 管理员辅助密码恢复使用独立 `identity.user.password-reset` 权限和必填审计原因；管理员不能为自己签发。服务只保存 256 位随机令牌的摘要，原文仅返回一次、30 分钟有效，重新签发立即废止旧令牌；确认时原子消费令牌、乐观更新凭证、撤销全部会话并写入带自助/管理员恢复分类及原因的密码变更 Outbox 事件
- `/me` 只从认证主体推导用户 ID 和会话作用域，返回 identity-service 自有的当前账号资料；固定字段形状与前端快照归一化共同防止切换作用域后残留旧身份数据
- 用户变更为 disabled、locked 或 closed 时，在状态乐观锁更新的同一事务撤销其全部有效会话；恢复 active 不复活旧会话，只允许重新登录
- 控制台显式退出先调用 identity-service 撤销当前服务端会话，再清除浏览器凭据；鉴权失败路径只做本地清理，避免退出接口形成递归 401
- 用户中心通过独立自助接口查看并撤销自己的登录会话，并展示登录时的客户端 IP 与 User-Agent；接口不接受目标用户或租户，所有权从 JWT 主体推导，并在撤销 SQL 中同时校验用户、会话和期望版本。管理员可指定目标的会话接口保持独立授权边界
- Access Token 过期时，控制台在所有服务请求间共享且仅共享“进行中”的一次 Refresh Token 轮换；每个原请求最多重放一次，新 Token 仍被拒绝则立即清除身份状态
...
- 手动触发必须携带任务期望版本；服务端在事务中锁定任务行，确认版本仍一致且任务处于启用状态后再创建执行记录
...
- 控制台以应用作为一级产品与代码边界：登录后先选择应用，只挂载当前应用的页面注册表、发布菜单和动态路由；`application_id` 同时进入权限、字典、搜索、审计及业务 API 上下文。平台管理也作为一个受 `__platform__` 约束的应用，而不是绕过应用模型的全局页面集合
...
- 我的任务实例筛选只返回服务端按当前用户、authorization-service 角色和领取关系计算后可见的实例；候选查询不得接受客户端角色声明，也不允许借筛选枚举应用下的其他流程实例
...
- 换套餐必须同时携带订阅版本和目标套餐版本；立即生效与下周期生效都在同一事务内先锁定 active 目标套餐，再以乐观锁更新订阅
- 用量价格写入和删除同时携带父套餐版本；新增、更新和删除都在同一事务内锁定 active 套餐后再变更子项，避免修改已变化或退役套餐的计费规则
- 发票、支付、提供商回调和退款具有持久化幂等边界；所有更新使用版本号乐观锁
- 控制台确认和作废账单前读取包含行项目的详情响应并正确解包账单实体；首次确认的详情版本、到期时间和幂等键必须跨模糊网络重试保留
- billing-center 提供独立“支付与退款”工作区：分页查询当前租户应用的支付尝试与退款，幂等发起支付和记录退款；支付方式引用只在提交时发送，不进入浏览器持久状态
- 发起支付使用支付权限域内的可支付账单远程选择接口，只搜索当前租户应用的 open 账单；请求携带账单版本，服务端在领取支付幂等记录的同一事务内锁定账单并按最新未付金额创建支付尝试
- 退款请求携带成功支付的版本；服务端同一事务内锁定支付和账单，先认领退款幂等记录，只有首次执行才校验并扣减可退款余额，重复请求返回原退款结果
...
- 版本号分配和持久幂等键在 rule-set 行锁内串行化；首次创建版本必须匹配规则集期望版本且规则集未禁用，幂等重放仍优先返回原结果；发布使用双版本乐观锁
...
- 投递查询通过投递域只读候选接口搜索 active 订阅，使用 `webhook.delivery:list` 权限；查看投递的账号无需额外获得订阅管理权限，控制台也不得要求操作员手填订阅 ID。
