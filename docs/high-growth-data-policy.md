...
| every event producer | published Outbox records | Service-specific, never shorter than the JetStream replay window; owning service | Event archive / object storage | Use existing native partitions where present; delete only `published_at IS NOT NULL` | Shared SQLStore producers plus identity and tenant complete; authorization legacy Store migration in progress |
