...
| identity-service | `mfa_login_challenges` | 24 hours after expiry or consumption; identity worker | Not archived; security events are exported separately | Deferred: short retention and bounded cleanup are sufficient until measured volume justifies time-bucketed identity | Implemented |
| identity-service | `password_reset_challenges` | 24 hours after expiry or consumption; identity worker | Not archived; completed password-change security events are exported separately | Deferred: short retention and bounded cleanup are sufficient; only token digests are retained | Implemented |
...
| every event producer | published Outbox records | Service-specific, never shorter than the JetStream replay window; owning service | Event archive / object storage | Use existing native partitions where present; delete only `published_at IS NOT NULL` | Implemented: every producer uses the shared SQLStore and bounded retention cleaner; webhook-service is consumer-only and removes its unused historical Outbox table |
