...
## 2026-09-02: action catalogs must describe product capabilities, not every backend method
...
## 2026-09-02: dependency injection does not prove infrastructure behavior executes
...
- Prevention: expose an explicitly authorized detail lookup for every mutable resource. After the user enters or confirms a mutation flow, fetch that detail, verify its persisted tenant/application ownership server-side, and take the expected version from the fresh result. Preserve the user's original target state: for toggle-style controls, abort and refresh when the current state no longer matches the state the user confirmed instead of deriving an opposite action from fresh state. Gate the UI flow with both read and mutation permissions; keep the backend optimistic-lock check authoritative for changes that occur after the detail read.
- Editing nuance: an edit form must fetch detail when the form opens and retain that version until submit, so a concurrent edit produces a conflict instead of being silently overwritten. Only no-form actions such as delete, revoke, rotate, publish, or cancel fetch detail immediately after confirmation. Enforce the reusable CRUD case with a static gate requiring every `BizCrudAdapter` that declares `update` to also declare `detail`.

## 2026-09-03: idempotency keys belong to user operations, not HTTP attempts

- Symptom: a payment or refund button generated `crypto.randomUUID()` inside its submit call, so a timeout followed by a user retry created a new idempotency namespace and could execute the financial operation twice.
- Root cause: duplicate-click protection and backend idempotency were treated as interchangeable. A loading guard suppresses concurrent clicks, but it does not preserve identity across sequential retries after an ambiguous network failure.
- Prevention: allocate the idempotency key when the user opens or starts the operation, retain it with the form until a confirmed success or an explicit new operation, and reuse it for every retry. Pair this with a synchronous submitting/keyed-action guard and disabled modal cancellation while a request is in flight. Unit-test that retry resolution calls the key generator exactly once; keep the backend scope and payload fingerprint authoritative.

## 2026-09-03: multi-stage operations need stable phase keys without caching ephemeral credentials
...
- MySQL deployment prerequisite: with binary logging enabled, trigger creation by a least-privilege migration identity fails with error 1419 unless `log_bin_trust_function_creators=1` is enabled. Grant the migration identity `TRIGGER` only for its service database, configure this server setting explicitly in development/CI and production infrastructure, and keep the runtime identity free of DDL privileges.
- Background persistence nuance: an Outbox dispatcher has no incoming user principal but still updates audited rows. Configure a stable service-scoped worker actor in the shared Outbox store; it must inject that actor on the same transaction connection before claim/success/failure updates. Do not weaken triggers or fabricate a user identity, and keep bounded physical retention as a separately reviewed terminal-row exception.
