...
- Tenant bootstrap/self-service operations that necessarily precede an active membership (create tenant, accept invitation, list the caller's tenants) remain authenticated and enforce their domain ownership rules without a tenant-scoped RBAC call. Tenant fact RPCs used by authentication/authorization (membership validation/read and organization-scope resolution) must not call authorization-service recursively; protected and intentionally excluded methods are both enumerated in tests.
