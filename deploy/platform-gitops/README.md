...
The service matrix also enables object storage for file, import, and export services. The corresponding `platform/<environment>/<service>` Vault records own both the internal storage endpoint and the externally resolvable presign endpoint alongside Region, bucket, credentials, and TLS flags; these values are deliberately not committed to Git.

Development, testing, staging, and production all receive the same 20-service matrix. Staging deliberately runs the backend `production` profile so TLS, credentials, and other production-only validation cannot be bypassed, while retaining its own Namespace, Vault paths, APISIX class, wildcard domain, and image promotion boundary. The console reports `staging` to the browser runtime and uses the matching staging subdomains.
